...
pub fn extract_url_payload(
    ...
    with_children: bool,
) -> (Option<String,>, Option<IndexMap<String, String,>,>, Option<String,>,) {
...
pub fn extract_id_parameter(url_string: String,) -> String {
    ...
    let mut param_id = single_paths.next().map(|r| r.to_string(),).expect("Should have param id",);
    ...
    if let Some(sub_string,) = parts_iter.next() {
        if sub_string.is_empty() {
            ...
            for pair in key_value {
                ...
                let key = sub
                    .next()
                    .expect(format!("we should have a key for the parameter key but got {}", url_string).as_str(),);
                let value = sub
                    .next()
                    .expect(format!("we should have a value for the key but got {}", url_string).as_str(),);
...
mod test {
    ...
    fn test_strings_with_id_param_and_children_and_query() {
        ...
        let task: UserTask = string
            ...
            .map(|(id, query, children,)| UserTask { id, query, children, },)
            .unwrap();
