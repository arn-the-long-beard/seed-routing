...
impl<Route: 'static + Debug + PartialEq + ParsePath + Default + Clone + Navigation,> Router<Route,> {
    ...
    pub fn set_handler<Msg: 'static,>(&self, orders: &mut impl Orders<Msg,>, callback: impl FnOnce(subs::UrlChanged,) + Clone + 'static,) -> &Self {
        self.clone()
    ...
    fn push_to_history(&self, route: Route,) {
        self.update_data(|mut data| data.push_to_history(route,),);
    }
    ...
    pub fn can_forward_with_route(&self,) -> Option<Route,> {
        ...
        let route = history
            .get(next_index,)
            .unwrap_or_else(|| panic!("We should have get route but index is failed {}", next_index),);
...
mod test {
    ...
    fn test_build_url() {
        ...
        router.navigate_to_url(url,);
        assert_eq!(router.current_route().clone(), ExampleRoutes::parse_path("").unwrap());

        let admin_url = router.base_url().clone().set_path("dashboard/admin/other".split('/',),);
        ...
        let admin_url = router.base_url().clone().set_path("dashboard/profile/1".split('/',),);

        router.navigate_to_url(admin_url,);
        assert_eq!(router.current_route(), ExampleRoutes::parse_path("/dashboard/profile/1").unwrap());
    }
    ...
    fn test_navigation_to_route() {
        ...
        assert_eq!(router.current_route(), ExampleRoutes::Dashboard(DashboardRoutes::Profile(1)));
        assert_eq!(router.current_history_index(), 0);

        router.navigate_to_new(ExampleRoutes::parse_path("/dashboard/profile/55",).unwrap(),);

        assert_eq!(router.current_route(), ExampleRoutes::Dashboard(DashboardRoutes::Profile(55)));
        assert_eq!(router.current_history_index(), 1);
        router.navigate_to_new(ExampleRoutes::Home,);

        assert_eq!(router.current_route(), ExampleRoutes::parse_path("").unwrap());
    ...
    fn test_backward() {
        ...
        assert_eq!(router.current_history_index(), 0, "We should have current index 0");
        ...
        assert_eq!(router.is_current_route(&ExampleRoutes::parse_path("dashboard/").unwrap()), true);
        ...
        assert_eq!(router.current_history_index(), 2);
        assert_eq!(router.current_route(), ExampleRoutes::parse_path("dashboard/admin/other").unwrap());
    }
